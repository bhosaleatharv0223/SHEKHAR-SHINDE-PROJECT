...
export function LoanStatusUpdate() {
  ...
  const communications = [
    { date: '07 Jun 2025', type: 'Call', badge: '#F59E0B', message: 'Called loan officer Mr. Shekhar Shinde, processing in progress', by: 'Agent Shekhar Shinde' },
    { date: '06 Jun 2025', type: 'Received', badge: '#16A34A', message: 'Bank acknowledged receipt, assigned ref: HDFC/2025/45678', by: 'System' },
    { date: '05 Jun 2025', type: 'Sent', badge: '#2563EB', message: 'Application documents submitted to HDFC Pune branch', by: 'Admin Shekhar Shinde' },
  ];

  const statusHistory = [
    { status: 'Sent to Bank', date: '05 Jun 2025 10:30 AM', by: 'Admin Shekhar Shinde', reason: 'All documents verified and complete', color: '#2563EB' },
    { status: 'Under Verification', date: '03 Jun 2025 02:15 PM', by: 'Admin Shekhar Shinde', reason: 'Documents received from client', color: '#16A34A' },
  ...
  const notes = [
    { text: 'CIBIL score 780 â€” excellent profile', by: 'Admin Shekhar Shinde', date: '02 Jun 2025' },
    { text: 'Bank processing expected 7 days', by: 'Admin Shekhar Shinde', date: '05 Jun 2025' },
  ];

  const getMessagePreview = () => {
    return `Dear Shekhar Shinde, Your Home Loan application status has been updated to: ${selectedStatus}. Our team will contact you shortly. â€” Sanskruti Associates`;
  };
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg text-gray-600">Shekhar Shinde â€” Home Loan â‚¹45,00,000</p>
            </div>
            <div className="text-right">
              <span className="inline-block px-4 py-2 bg-[#2563EB] text-white rounded-full mb-2">
                Sent to Bank
              </span>
              <p className="text-sm text-gray-500">Updated: 05 Jun 2025 by Admin Shekhar Shinde</p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    <option>ðŸ†• New Lead</option>
                    <option>ðŸ“‹ Documents Pending</option>
                    <option>ðŸ” Under Verification</option>
                    <option>ðŸ¦ Sent to Bank</option>
                    <option>âš™ï¸ Bank Processing</option>
                    <option>âœ… Loan Sanctioned</option>
                    <option>ðŸ’° Amount Disbursed</option>
                    <option>âŒ Rejected</option>
                    <option>ðŸ”’ Case Closed</option>
                  </select>
                </div>
                <div>
                ...
                <p className="text-sm text-[#F59E0B] flex items-center gap-2">
                  âš ï¸ This will notify the applicant via SMS and Email automatically
                </p>
              </div>
            </div>
                  ...
                  <div key={index} className="p-4 border border-gray-200 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="px-3 py-1 text-xs text-white rounded-full" style={{ background: comm.badge }}>
                        {comm.type === 'Sent' && 'ðŸ“¤'} {comm.type === 'Received' && 'ðŸ“¥'} {comm.type === 'Call' && 'ðŸ“ž'} {comm.type}
                      </span>
                    </div>
                ...
                {documents.map((doc, index) => (
                  <label key={index} className="flex items-center gap-3 cursor-pointer">
                    ...
                    <span className={`text-sm ${doc.received ? 'text-[#16A34A]' : 'text-[#EF4444]'}`}>
                      {doc.name} â€” {doc.received ? 'Received' : 'Not Received'}
                    </span>
                  </label>
                ))}
              </div>
              ...
              <div className="space-y-3 mb-4">
                {notes.map((note, index) => (
                  <div key={index} className="p-3 bg-gray-100 rounded-lg">
                    ...
                    <p className="text-xs text-gray-500">{note.by} â€” {note.date}</p>
                  </div>
                ))}
              </div>
      ...
      <div className="fixed bottom-0 left-[260px] right-0 bg-white border-t border-gray-200 p-4 shadow-lg z-30">
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-600">
            Applicant: <span className="text-[#1F2937]">Shekhar Shinde</span> |
            Loan: <span className="text-[#1F2937]">Home Loan</span> |
            Amount: <span className="text-[#1F2937]">â‚¹45,00,000</span>
          </div>
