...
const kycDocuments = [
  { key: 'Aadhaar Card (Applicant)', label: 'Aadhaar Card (Applicant)', required: true },
  { key: 'Aadhaar Card (Co-applicant)', label: 'Aadhaar Card (Co-applicant)', required: false },
  { key: 'PAN Card (Applicant)', label: 'PAN Card (Applicant)', required: true },
  { key: 'PAN Card (Co-applicant)', label: 'PAN Card (Co-applicant)', required: false },
  { key: 'Bank Statement 6 Months', label: 'Bank Statement 6 Months', required: true },
  { key: 'Photo', label: 'Photo', required: true },
  { key: 'Cheque', label: 'Cheque', required: true },
];

const incomeDocuments = [
  { key: 'Salary Slip 3 Months', label: 'Salary Slip 3 Months', required: true },
  { key: 'Joining Letter or Form 16', label: 'Joining Letter or Form 16', required: true },
];

const propertyDocuments = [
  { key: '8A Utara', label: '8A Utara', required: true },
  { key: 'Chatuhsima Dakhla', label: 'Chatuhsima Dakhla', required: true },
  { key: 'Plan Estimate and Blue Print', label: 'Plan Estimate and Blue Print', required: true },
  { key: '7/12 Utara and Ferfar', label: '7/12 Utara and Ferfar', required: true },
  { key: 'Gharpatti Receipt', label: 'Gharpatti Receipt', required: true },
  { key: 'Sale Deed or other ownership proof', label: 'Sale Deed or other ownership proof', required: true },
];
...
function UploadBox({ documentItem, fileName, onUpload }) {
  const inputId = `home-upload-${documentItem.key.replace(/\s+/g, '-').toLowerCase()}`;
...
function UploadSection({ title, documents, fileMap, onUpload, headerClassName }) {
  return (
...
export function HomeLoanForm() {
  ...
  const documents = [...kycDocuments, ...incomeDocuments, ...propertyDocuments];
  ...
  const handleOtpChange = (index, value) => {
    ...
    if (digit && index < 5) {
      document.getElementById(`home-loan-otp-${index + 1}`)?.focus();
    }
  };
        ...
        <div className="mx-auto rounded-[24px] border border-white/95 bg-white/90 p-6 shadow-[0_20px_60px_rgba(37,99,235,0.15)] backdrop-blur-[20px] sm:p-8 [animation:loanStepIn_0.3s_ease-in-out_both]">
          ...
          {currentStep === 5 && (
            <div className="[animation:loanStepIn_0.3s_ease-in-out_both]">
              ...
              <p className="mb-6 text-gray-600">Documents shown for Home Loan</p>
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white/80 p-5 shadow-sm">
                  ...
                  <div className="grid gap-3 text-gray-700 md:grid-cols-2">
                    ...
                    <p>Monthly Income: Rs. {formData.monthlyIncome}</p>
